...
# change epoch
$run examples/mnist/train_mnist.py --gpu=-1 --epoch=1 --unit=10
$run examples/mnist/train_mnist.py --gpu=0 --epoch=1 --unit=10
if [ -f examples/mnist/train_mnist_custom_loop.py ]; then
  $run examples/mnist/train_mnist_custom_loop.py --gpu=-1 --epoch=1 --unit=10
...
$run examples/ptb/train_ptb.py --gpu=-1 --epoch=1 --unit=10 --test
...
$run examples/sentiment/train_sentiment.py --gpu=-1 --epoch=1 --batchsize=1 --epocheval=1 --test
...
$run examples/imagenet/train_imagenet.py --test --gpu -1 -a nin -R ../data/imagenet -B 1 -b 1 -E 1 $imagenet_data $imagenet_data
$run examples/imagenet/train_imagenet.py --test --gpu -1 -a alex -R ../data/imagenet -B 1 -b 1 -E 1 $imagenet_data $imagenet_data
$run examples/imagenet/train_imagenet.py --test --gpu -1 -a googlenet -R ../data/imagenet -B 1 -b 1 -E 1 $imagenet_data $imagenet_data
$run examples/imagenet/train_imagenet.py --test --gpu -1 -a googlenetbn -R ../data/imagenet -B 1 -b 1 -E 1 $imagenet_data $imagenet_data
...
$run examples/word2vec/train_word2vec.py -e 1 -b 10 --gpu=-1 --test
echo "it" | $run examples/word2vec/search.py
$run examples/word2vec/train_word2vec.py -e 1 -b 10 --gpu=0 --test
echo "it" | $run examples/word2vec/search.py
$run examples/word2vec/train_word2vec.py -e 1 -b 10 -m cbow --out-type ns --gpu=-1 --test
echo "it" | $run examples/word2vec/search.py
$run examples/word2vec/train_word2vec.py -e 1 -b 10 -m cbow --out-type ns --gpu=0 --test
echo "it" | $run examples/word2vec/search.py
$run examples/word2vec/train_word2vec.py -e 1 -b 10 --out-type original --gpu=-1 --test
echo "it" | $run examples/word2vec/search.py
...
$run examples/vae/train_vae.py -e 1 --gpu=-1 --test
$run examples/vae/train_vae.py -e 1 --gpu=0 --test

# dcgan
echo "Runnig DCGAN example"

$run examples/dcgan/train_dcgan.py -b 1 -e 1 --gpu=-1 -i ../data/dcgan --n_hidden=10 --snapshot_interval 1 --display_interval 1
...
if [ -f examples/seq2seq/seq2seq.py ]; then
  ...
  $run examples/seq2seq/seq2seq.py ../data/seq2seq/source.txt ../data/seq2seq/target.txt ../data/seq2seq/source.vocab.txt ../data/seq2seq/target.vocab.txt --unit 8  --validation-source ../data/seq2seq/source.txt --validation-target ../data/seq2seq/target.txt ${VALIDATION_OPTS} --gpu=-1
